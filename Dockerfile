...
RUN apt-get -y update &&\
  ...
  apt-get install -y --no-install-recommends \
    ...
    automake autoconf libtool libpthread-stubs0-dev libva-dev libdrm-dev &&\
  ...
  apt-get remove --purge \
    git curl llvm-3.4-dev automake autoconf libtool libpthread-stubs0-dev libva-dev libdrm-dev &&\
